...
import StreamCard, { StreamCardSkeleton } from '../components/home/StreamCard';
...
const LivePage = () => {
  ...
  useEffect(() => {
    const fetchStreams = async () => {
      ...
      setError(null);
      try {
        const data = await streamService.getAllStreams({ status: 'live' });
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-white min-h-screen">
      ...
      <main className="flex-1 overflow-y-auto bg-white py-8 px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12">
        <h1 className="text-3xl font-bold mb-8 text-neutral-800">Live Now</h1>
        {isLoading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {Array(8).fill(null).map((_, index) => <StreamCardSkeleton key={`skel-live-${index}`} />)}
          </div>
        )}
        {error && <p className="text-red-500">{error}</p>}
        {!isLoading && !error && streams.length === 0 && (
          <p className="text-neutral-600">No streams are live right now. Check back later!</p>
        )}
        {!isLoading && !error && streams.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {streams.map(stream => (
